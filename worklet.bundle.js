(() => {
  ...
  // worklet.js
  var COLOR_PROPS = [...Array(8)].map((_, i) => `--avatar-color-${i + 1}`);
  ...
  var VoronoiAvatar = class {
    ...
    paint(ctx, geometry, props) {
      ...
      ho(seed);
      const focus = {
        x: po(0, width, width / 2),
        y: po(0, height, height / 2)
      };
      const points = [...Array(At(4, 12, true))].map(() => {
        return {
          x: po(0, width, focus.x, 0),
          y: po(0, height, focus.y, 0)
        };
      });
      const { cells } = Qi({
        width,
      ...
      ctx.lineWidth = 24;
      cells.forEach((cell) => {
        const choice = At(["line", "circle", "cell"]);
